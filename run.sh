...
CUDA_VISIBLE_DEVICES=0 python eval_llm.py \
    ...
    --epochs=$epochs --lr_drop=$lr_drop --n_gpus=$n_gpus \
    --threshold=0.80 --n_viz=0 \
    --load_path_reward=$load_path_reward --gain_fn=exp --train_ranker \
    --load_path=$load_path --eval --eval_llm --llm_exp --output_folder=$model --eval_offline
