...
def get_query_groups(df_path, locale="us"):
    """Groups query and product data from a Parquet file.

    Args:
        df_path (str): Path to the input Parquet file.
    ...
    for query_id, group in df.groupby("query_id"):
        ...
        QG[query_id] = {"query_id": query_id, "query": query_text, "products": products}
    return QG
...
def run(args):
    ...
    query_groups = get_query_groups(args.data_path)
...
def process_esci(args):
    ...
    unique_queries = df_sampled[["query_id", "query"]].drop_duplicates()
    unique_queries["query_language"] = unique_queries["query"].apply(fasttext_detect)

    # Merge language back into the main dataframe
    df = df_sampled.merge(unique_queries[["query_id", "query_language"]], on="query_id", how="left")

    # Filter for English queries
    df_sampled_en = df[df["query_language"] == "en"]
    df_sampled_en.to_parquet(args.data_path, index=False)
...
def main():
    ...
    parser.add_argument("--data_path", type=str, default="/home/gbhatt/scratch/ranking/data/esci_data/df_sampled_en.parquet", help="Path to save processed esci data")
